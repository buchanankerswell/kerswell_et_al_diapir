...
Before deriving the details of marker classification, we hypothesize that subducted and recovered markers may be distinguished by their PTt paths. If true, clustering algorithms like GMM may reliably classify markers by (dis)similarity along any number of dimensions computed from marker PTt paths.
...
### Feature selection and final decision
...
## Classification variance and error

Typical measures of classification accuracy, like confusion matrices [@stehman1997], bootstrap [@efron1994], and cross-validation [@kohavi1995], leverage *a priori* information (class labels $y_i$) about the data $x_i$ to validate clustering results. Generally speaking, these methods compare classes predicted by clustering $z_i$ with actual classes $y_i$ to quantify type I (false positive---classified as recovered but was subducted) and type II (false negative---classified as subducted but was recovered) errors. However, our markers are unlabelled (no $y_i$) so *true* validation is not possible by these methods or similar. Instead, we use a straight-forward resampling approach to assess variance in our clustering results. First, all markers $x_i$ from all numerical models are classified one-hundred times. Summary statistics for each numerical model (mean and standard deviation) are then computed from the one-hundred classifications ([@tbl:marx.summary]).
...
rename(
  ...
  '$\\sigma_{rec}$' = sd.rec,
  '$\\sigma_{sub}$' = sd.sub,
  '$\\frac{n_{rec}}{n_{marx}}$' = mean.ratio,
  'age\n$[Ma]$' = age,
  '$\\sigma_{\\frac{n_{rec}}{n_{marx}}}$' = sd.ratio,
) %>%
pander::pandoc.table(
  split.tables = Inf,
  keep.line.breaks = T,
  round = c(rep(0, 9), 2, 3),
  caption = 'Summary of subduction parameters and marker tracing results {#tbl:marx.summary}',
  missing = '**'
)
```

Type I and type II errors, qualitatively assessed by visualizing the geodynamic evolution and marker metamorphic conditons of each model (see [@sec:vis]), show consistently low prevalence of type I error and variable amounts of type II error. Note that type II error is analogous to a biased rock record only the proportion of recovered rocks that are exhumed to Earth's surface may be sampled. Therefore, some principled comparisons may still be made between the rock record and models with abundant type II error. The same is not true for type I error. Rocks do not generally record anomalously high PT conditions if demonstrably absent of disequilibrium (kinetic) textures. The only example of excessive type I error is shown in [@fig:typeI; @fig:cdm46snaps; @fig:cdm46meta; @fig:cdm46class]. Type I error is especially apparent in [@fig:typeI]b with eighty percent of markers being recovered from pressures of $\leq 6~GPa$. Model cdm46 notwithstanding, the prevalence of type I error is low and overall and the shapes of cumulative probability curves for all other models are negligibly affected.

![Example of excessive type I error.](../figs/typeI.png){#fig:typeI}

\clearpage

## Marker motions

![Examples of mantle wedge diapirs.](../figs/diapirs.png){#fig:diapir}

\clearpage

## Marker proportions

![Geodynamic evolution for model cdf78.](../figs/cdf78_snaps.png){#fig:snaps}

![Maximum metamorphic conditons for markers recovered from model cdf78. Maximum pressures for recovered markers (solid curves) range from `r round(head(maxP.cdf78$maxP[maxP.cdf78$cdf <= 0.8], 1)/1e4, 1)` to `r round(tail(maxP.cdf78$maxP[maxP.cdf78$cdf <= 0.8], 1)/1e4, 1)` (a) and only marginally overlap with the rock record (dotted curves). However, maximum temperatures for recovered markers range from `r round(head(maxT.cdf78$maxT[maxT.cdf78$cdf <= 0.8], 1)-273)` to `r round(tail(maxT.cdf78$maxT[maxT.cdf78$cdf <= 0.8], 1)-273)` (b) and show better agreement with the rock record than (a). PD15 dataset from @penniston2015.](../figs/cdf78_meta.png){#fig:meta}
...
for(i in seq_along(models)){
  cat('\n\n![Marker classification for model ', models[i], '.](../figs/', models[i], '_class.png){#fig:', models[i], 'class}', sep = '')
	cat('\n\n![Metamorphic conditions of markers recovered from model ', models[i], '.](../figs/', models[i], '_meta.png){#fig:', models[i], 'meta}', sep = '')
  cat('\n\n![Geodynamic evolution of model ', models[i], '.](../figs/', models[i], '_snaps.png){#fig:', models[i], 'snaps}', sep = '')
  cat('\n\n\\clearpage')
}
```
